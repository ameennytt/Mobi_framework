...
window.Gameplay = (function () {
  ...
  function resolveRound() {
    ...
    // Half-time: TV break screen + a "pass the phone" handoff countdown on the controller.
    if (!finished && !halfShown && score.round >= Math.ceil(score.rounds / 2)) {
      halfShown = true;
      if (T().renderTVBreak) {
        T().renderTVBreak({
          kind: 'innings', title: 'Half Time', sub: 'Second half coming up',
          stats: [{ label: 'You', value: score.you }, { label: 'CPU', value: score.cpu }, { label: 'Round', value: `${score.round}/${score.rounds}` }],
          seconds: 3,
        });
      }
      game.send('handoff', { title: 'Half Time', next: 'Second half', seconds: 3 });
    }
