...
| `framework/renderer/shot-visuals.js` | `ShotVisuals` | Ball/projectile math |
...
(or `config.field`). Each is `(ctx,W,H)` placed with `ShotVisuals.perspective`.
...
## ShotVisuals — `framework/renderer/shot-visuals.js`

```js
ShotVisuals.perspective(W,H);               // {cx,horizY,farY,nearY,farW,nearW}
ShotVisuals.computeShotLanding(msg,W,H,'right');
ShotVisuals.computeArcParams(msg,W,H);      // {dur,arcH,big,grounded}
ShotVisuals.buildVisual(msg,{W,H,tvTarget,tvOvers,roomCode}); // full block
ShotVisuals.calcWinProb(score,target,overs);
```
