# app-template — debug APK build

The React Native shell template. `npm run publish <id>` copies this to
`games/<id>/app/` with that game embedded and `App.tsx` pointed at it. Build from the
generated `games/<id>/app/` (not from this template). Phone = the brain (runs the
embedded Node server); the TV is a browser it feeds.
...
## Build + run (debug)
```bash
npm run publish <id>          # from repo root → generates games/<id>/app
cd games/<id>/app
...
`http://localhost:3000/games/<id>/lobby.html` in the WebView.
