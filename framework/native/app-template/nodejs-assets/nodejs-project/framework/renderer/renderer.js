...
class FrameworkRenderer {
  ...
  init(canvasOrId, drawFn) {
    this.canvas = (typeof canvasOrId === 'string')
      ? document.getElementById(canvasOrId)
      : canvasOrId;
    if (!this.canvas) {
      console.warn(`[Renderer] Canvas not found:`, canvasOrId);
      return;
    }
    this.ctx = this.canvas.getContext('2d');

    // Convenience: a single draw callback is registered as the 'object' layer so
    // simple games can do FrameworkRenderer.init('id', draw) without manually
    // wiring FrameworkLayers.
    if (typeof drawFn === 'function' && window.FrameworkLayers) {
      window.FrameworkLayers.register('object', drawFn);
    }
