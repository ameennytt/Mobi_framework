...
/**
 * Framework HTTP server (Node).
 *
 * Serves the static framework + game files (`/framework/*`, `/games/*`) and a few
 * pairing/diagnostic routes, then hands the same HTTP server to the WebSocket
 * layer for the phone↔TV relay. Binds 0.0.0.0:3000 so a phone on the LAN (or the
 * embedded nodejs-mobile runtime) can reach it.
 *
 * Security: every requested path is resolved and confined to GAMES_ROOT /
 * FRAMEWORK_ROOT (path-traversal guard) — see the containment check below.
 * Native hook: on listen it calls global.__rnBridge (set by the RN shell) so the
 * app learns the server is ready. window globals: none (this is server-side).
 */
...
const FRAMEWORK_ROOT = path.resolve(__dirname, '..', '..', 'framework');
...
function requestHandler(req, res) {
  ...
  // (e.g. /games/x/../../../secret). Server binds 0.0.0.0, so this matters.
  if (localFile && allowedRoot) {
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  // Native shell hook: signal React Native the embedded server is up (no-op in dev).
  if (global.__rnBridge) { try { global.__rnBridge.send('server-ready'); } catch (_) {} }
});
