...
- **`Projectile`** (`framework/renderer/projectile.js`) — sport-neutral geometry
  (`perspective`, `boundaryHit`, seeded RNG, `arc`) so a ball lands exactly on the drawn
  boundary. (Cricket-specific shot math lives in the game — `games/chase/chase-shot.js`.)
...
## 7b. Shipping a game
```bash
npm run publish <id>   # generates games/<id>/app/ (self-contained RN+Android + embedded server)
```
Then build the APK from that game's own app (`cd games/<id>/app && npm install &&
npx react-native run-android`) and, for cloud pairing, deploy the worker
(`cd framework/rendezvous && wrangler deploy`) and set `RENDEZVOUS_URL` in the game
config. Each game owns its app; `framework/native/app-template/` is the template publish
copies from. See `framework/native/README.md` and `framework/rendezvous/README.md`.
